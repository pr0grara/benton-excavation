...
The current images are **cloned placeholder photos** (carried over from the template and recycled
across the eight service slugs) so the build resolves and the site renders. They are generic stock —
none of them shot in Benton County, and several slugs reuse the same photo. **Replace them with real
job photos before launch** — real local proof (your actual machines, a pad you graded near
Bentonville, a driveway you built, a pond you dug) is load-bearing for this kind of site
...
| `services/excavation-services.jpg` | `excavation-services` service (hub catch-all) |
| `services/land-grading.jpg` | `land-grading` service |
| `services/site-preparation.jpg` | `site-preparation` service |
| `services/driveway-installation.jpg` | `driveway-installation` service |
| `services/drainage-solutions.jpg` | `drainage-solutions` service |
| `services/land-clearing.jpg` | `land-clearing` service |
| `services/pond-excavation.jpg` | `pond-excavation` service |
| `services/dirt-work-hauling.jpg` | `dirt-work-hauling` service |
